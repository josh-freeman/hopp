...
5. **Canonical phone flow (user simplification, 2026-09-13):** Plan → Results with the route map, budget, directions, source details and regular fallback together → one **Go live** action → Live, where **Start sprint** records leaving the feeder and **On the platform** records self-confirmation. Settings is optional. There are six screens: Plan, Results, Live, Settings, On the platform and optional Profile. The map is prominent in Results and needs no separate preview tap; route notes expand in place. Legacy `#try`, `#route`, `#detail` and `#shortcut` hashes resolve to `#results`. The original mockups in `docs/design/phone-flow/` document the earlier proposal and no longer define the navigation.
6. **Hosting:** The frontend uses GitHub Pages under `/hopp/`; the optional account API uses the existing website backend at `https://api.joshfreeman.me/hopp`. Anonymous planning requires no account backend. The UI is English-only.
7. **Discovery and copy (user correction, 2026-09-13):** Show shortcuts in the context of the searched trip. No prominent, easily accessible list of all routes. Route notes apply only to the selected connection. Use plain functional copy. The later explicit user-selected lead “the art of being on time” and descriptor “Train shortcuts for runners.” are retained; they do not authorize additional promotional slogans.
...
9. **Visual design (user correction, 2026-09-13):** Treat the map and timing as the main content. Keep the typography deliberate, the hierarchy compact, actions thumb-reachable and copy functional. Geographic context must come from the researched source data; a diagram cannot imply an unverified physical path.

10. **Identity and graphics (later user direction, 2026-09-13):** Use “the art of being on time” and “Train shortcuts for runners.” on Plan. Use SVG interface symbols instead of emoji. Early-alighting offers lead with **GET OFF EARLY AT** and identify the earlier stop relative to the regular station stop; do not use that instruction for an origin start.
11. **Shared accounts:** Optional Google sign-in uses the website user system while issuing a separately scoped Hopp session. Existing website sessions are validated server-side. Hopp signup does not grant website roles or membership. Profile stores nickname, preferences, selected style, progress and private self-reported trip notes. Hopp sign-out preserves the website session; Hopp deletion preserves the website account/access and revokes dedicated Hopp sessions.
12. **Implemented rewards:** Three server-graded practice questions award **25 points each once**. One successful authenticated connection check per **Europe/Zurich calendar day** awards **5 points**; repeated checks update last-seen time but add zero points. Themes unlock at Signal **0**, Forest **25**, Night **75**, Track **150**, Alpine **300** and Dusk **600**. They change the profile card, not the planner or safety colors. Prepared records all three practice questions; active-day milestones at **7/30/100** are cumulative, with no streak penalty or expiry.
13. **Evidence and engagement:** Start sprint and On the platform are self-reports. Saved attempts remain **unverified and worth zero points**; no GPS trail or train-boarding verifier is implemented. Points for connection checks measure app use, not travel completion. Admin-only DAU/WAU/MAU/YAU count distinct authenticated accounts within rolling **1/7/30/365 UTC-day** activity windows; activation means an account has recorded a connection check. Aggregate reporting also includes activation rate, DAU/MAU stickiness and exact-day D7/D30 retention over thirty eligible Swiss signup dates, excluding incomplete follow-up days and using null for an empty denominator. Daily activity sends no route, search text or location trail.
14. **Scope and validation:** Profile adds one optional screen without extending Plan → Results → Go live. Mock accounts are entirely separate from real website/Hopp sessions. Strava imports, calibration and Strava-derived rewards are deferred under the current API Policy. Automated account, transport and browser checks do not constitute a complete human Google consent test or a physical station trial. Current test totals and pending work are maintained in [Phone QA](../../qa/README.md).

Status: the planner, simplified interface and shared-account/progress implementation are complete in source. The real account API is not yet live: Fly billing blocked deployment and production Hopp endpoints return 404. The frontend keeps unavailable sign-in inside Hopp; isolated demo accounts work. Release checks and deployment evidence are recorded separately in the QA guide. The later sections preserve the earlier design proposal; §0 and [the current account contract](../../ACCOUNTS.md) supersede their device-ID identity, proposed Worker/D1 backend, half-points, speed-based scoring, Strava calibration and unimplemented verification mechanics. Date: 2026-09-13.

---

The visual pass following this correction uses locally hosted Instrument Sans and IBM Plex Mono, a warm neutral surface, dark primary actions, distinct platform signage, and 26 consistent SVG area maps. A Watch video link on Plan opens the published demo player.
...
│  ui/   plan · results (map + timing) · live · settings · done                                               │
...
The current flow is **Plan → Results → Live**, plus Settings and On the platform. It implements §0.5 and supersedes the original multi-step mockups. Route discovery remains part of a trip search; there is no directory of all routes.

The single-page hash router has five screens. Interactive targets are at least 44 × 44 CSS px, text is at least 12 px, inputs are at least 16 px, and primary actions sit in the lower viewport quarter. The layout supports 320 px width, dark mode and reduced motion. GO status is conveyed by text as well as colour.

1. **Plan:** From and To autocomplete, nearest-stop lookup, departure time and recent destinations. **Find connections** searches with the saved default pace and margin; no account or settings step is required. A clearly labelled demo uses synthetic times without timetable requests.
2. **Results:** The sprint offer identifies the public train number, departure, alight stop, target platform and earlier arrival. Have/need/margin and spare after margin appear with a prominent route map. Directions follow the map; route notes and sources expand inline. The regular fallback and other connections remain on the same page. **Go live** is the single primary continuation; **Not now** keeps regular connections. No matching GO means no sprint action. Stale or expired offers lose their GO action automatically. Map context is bundled from the researched evidence; it does not establish indoor positioning or current entrance availability.
3. **Live:** Show the alight/start instruction, chosen service, platform, locally ticking countdown and full budget. Keep the fallback and primary action visible without scrolling at the minimum tested viewport. **Start sprint** distinguishes leaving the feeder from simply viewing guidance; the action then becomes **On the platform**. Live updates preserve the selected train identity. Invalid or stale timing changes the instruction and pauses the countdown as appropriate.
4. **On the platform:** Show the selected train and ask the passenger to check the departure board. This is self-confirmation, not verified boarding, a timed field result or an award of points. **Plan another trip** returns to Plan.
5. **Settings:** Optional offer toggle, single sprint pace, luggage adjustment and minimum spare time, saved locally. No pace-tier chooser or account setup is required.

Legacy `#try`, `#route`, `#detail` and `#shortcut` URLs canonicalize to `#results`. Sources belong to the matched route and can be read before opening Live. Read route instructions before moving; keep to public crossings and signals and never cross railway tracks. Timing is an estimate rather than a guaranteed connection.
...
- **Verification is GPS, never a self-report** (user, 2026-09-13). "On the platform" is only a stopwatch tap; no points come from it. An attempt is *accepted* only when all three hold: (1) **start** — the phone leaves the alight stop's geofence (`alight.geofence`, r 40 m, outdoors, reliable) within 90 s of the tram's real-time departure from that stop (stationboard prognosis), which becomes `t0`; (2) **finish** — the phone is inside the route's `landing` geofence *or* GPS is unavailable there (underground platforms 31–34 / 41–44 and the halls block GPS, so absence is not a failure); (3) **on the train — movement × timetable** (user, 2026-09-13): between 2 and 6 min after the target train's *real-time* departure `depTs`, ≥ 3 GPS fixes must match the train's expected position, not just "a train on this line". Expected position at time `t` is interpolated along `station.departureCorridor` (OSM railway polyline to the next stop, stored per hack) from `depTs` at 0 m to the next stop's real-time arrival (from the connection's `passList`, prognosis if present) at the corridor's length, with a 40 s dwell at each end. A fix matches when its along-track position is within 300 m of the expected one, its cross-track distance to the corridor ≤ 150 m, and its speed ≥ 50 km/h. Three matching fixes ⇒ verified; a phone on the next S-Bahn two minutes later is systematically ~1.5 km behind and fails; a phone still on the tram or on foot fails on speed and corridor. `engine/verify.ts` is pure (fixes + corridor + timetable in, verdict out) and unit-tested with synthetic tracks (on the train, on the following train, on the tram, stationary). `actualS = t_platform − t0` where `t_platform` is the tap if it exists and the geofence entry otherwise; the "made it" bonus needs (3). Web apps cannot read GPS in the background, so the Go-live screen simply stays open after departure and shows "verifying you're on the IC 1…" until (3) passes (typically 2–4 min after the doors close) — then the result sheet. Attempts without (1) or (3) are stored as `unverified` (personal stats, half points, never on a leaderboard). Server checks the same rules again (`workers/api/src/verify.ts`); the client only pre-screens.
...
- **Where it appears in the flow**: Go-live → "On the platform" (stopwatch tap) → "verifying you're on the train…" (GPS, 2–4 min) → result sheet: your time vs SBB's budget (race bar), points earned, rank on this shortcut, badge; any future leaderboard must remain tied to a relevant route, rather than restoring browse-all discovery. Nothing in the MVP flow changes; the MVP ships `calibration = null`, no points and no GPS verification (the geofence/corridor fields exist in the schema as optional so phase 2 is a data + backend change).
...
1. **Week 2 — PWA + entry path:** service worker (shell offline), wake lock, install prompt, GPS nearest-hack + "I'm on the T2 08:24" pick, trip-specific share/permalink (hash-encoded hack/destination/tram + Web Share button).
