...
export function accountScreen(state: AccountState, view: AccountView): string {
  ...
  if (!account) {
    return `<section class="screen account-screen" data-screen="account" data-testid="screen"><div><p class="eyebrow">Optional profile</p><h1>Your Hopp profile</h1></div>
      ${message}${state.status === 'loading' ? '<p class="muted" role="status">Loading your profile…</p>' : ''}
      <div class="profile-pass theme-signal"><span class="eyebrow">Hopp</span><h2>Optional.<br>Free. Private.</h2>${profileGraphic}</div>
      <ul class="account-benefits"><li>Your pace and margin on every device.</li><li>Three practice checks, points once each.</li><li>Saved trips stay private and unverified.</li></ul>
      <p class="muted">Sign in with Google. Search and live guidance never need an account.</p>
