...
export function fallback(result: PlanResult, compact = false, alreadyOff = false): string {
  ...
  if (!connection) return '<aside class="fallback" data-testid="fallback"><div><strong>SBB connection also at risk</strong><p>Refresh connections or check the departure board.</p></div></aside>';
