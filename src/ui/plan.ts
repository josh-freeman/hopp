...
export function planScreen(profile: Profile, from: string, to: string): string {
  ...
  return `<section class="screen plan-screen" data-screen="plan" data-testid="screen">
    ...
    <form id="trip-form" class="trip-form">
      ...
      <div class="bottom-actions"><div id="plan-message" class="inline-message" role="status"></div><button type="submit" class="primary" data-testid="primary-action">Find connections ${icon('arrow')}</button><div class="demo-links"><a class="text-link demo-link" href="?mock=1#plan">Example trip</a><span aria-hidden="true">·</span><a class="text-link demo-link" href="${import.meta.env.BASE_URL}demo.html">Watch video</a></div></div>
    </form>
  </section>`;
}
