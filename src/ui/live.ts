...
export function liveScreen(result: PlanResult, selected: Candidate, now: number, running: boolean): string {
  ...
  return `<section class="screen live-screen ${active ? '' : 'live-paused'}" data-screen="live" data-testid="screen"><div class="live-train"><span>${esc(trainName(selected))} · ${t(formatTime(selected.departureTs))}</span><span class="live-indicator">${stale ? 'PAUSED' : selected.live ? 'UPDATED' : 'SCHEDULED'}</span></div>
