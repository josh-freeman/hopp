...
export function liveScreen(result: PlanResult, selected: Candidate, now: number, running: boolean): string {
  ...
  return `<section class="screen live-screen ${active ? '' : 'live-paused'}" data-screen="live" data-testid="screen"><div class="live-train"><span>${esc(trainName(selected))} · ${t(formatTime(selected.departureTs))}</span><span class="live-indicator">${stale ? 'PAUSED' : selected.live ? 'LIVE' : 'TIMETABLE'}</span></div>
    ...
    <div class="live-numbers"><div><span class="eyebrow">Platform</span><strong data-testid="platform">${esc(selected.platform || '—')}</strong></div><div><span class="eyebrow">Until doors close</span><strong data-testid="countdown" ${active ? '' : 'aria-label="Countdown paused"'}>${active ? t(formatDuration(Math.max(0, runningTime))) : '—:—'}</strong></div></div>
    <div class="live-budget">${active && !running ? `<p class="live-spare"><strong>${formatDuration(Math.max(0, available - selected.sprintS - selected.marginS))}</strong> to spare</p>` : ''}${budget({ ...selected, haveS: running ? selected.haveS : available })}${active && !running ? '' : `<p>${running ? 'Budget at alighting · follow the crossings and signals' : stale ? 'No fresh update for over 2 minutes. Countdown paused.' : passed ? 'You have passed the stop. Search for a new connection.' : 'The sprint no longer leaves the planned margin.'}</p>`}</div>
