...
export function routeVisualization(h: Hack, c: Candidate): string {
  ...
  const visual = areaMap ? `<figure class="route-map"><img src="${import.meta.env.BASE_URL}maps/${esc(mapFile)}" alt="Area map of ${esc(h.station.name)}: ${esc(h.alight.name)} and ${c.route?.landing ? `Platform ${esc(c.platform)} access` : 'station centre'}"/><figcaption>A ${esc(shortStop(h.alight.name))} · ${c.route?.landing ? `B Platform ${esc(c.platform)} access` : 'S Station centre; platform access not mapped'}<br>Area map · © OpenStreetMap contributors</figcaption></figure>`
