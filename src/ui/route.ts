...
export function routeVisualization(h: Hack, c: Candidate): string {
  ...
  const visual = areaMap ? `<figure class="route-map"><img src="${import.meta.env.BASE_URL}maps/${esc(mapFile)}" alt="Area map of ${esc(h.station.name)}: ${esc(h.alight.name)} and ${c.route?.landing ? `Platform ${esc(c.platform)} access` : 'station centre'}"/><figcaption>A ${esc(shortStop(h.alight.name))} → B ${c.route?.landing ? `Platform ${esc(c.platform)} entrance` : 'station centre · platform access not mapped'} · <span class="nowrap">© OpenStreetMap contributors</span> · <a class="route-map-link" href="${osm}" target="_blank" rel="noopener">Open map ${icon('arrow')}</a></figcaption></figure>`
